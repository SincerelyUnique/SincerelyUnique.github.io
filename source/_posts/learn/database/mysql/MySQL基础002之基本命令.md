---
title: MySQL基础002之基本命令
date: 2022-10-08 12:54:43
tags:
categories:
- [学习, 数据库学习, MySQL]
valine:
...
# 3.使用着重号查询，为什么使用着重号？ 为了区分关键字, 有时候执行sql碰到关键字会报错
...
select salary as "out put" from employees; -- 建议
...
/*
 ...
 select 100 + 99; 两个操作数都是数值型，做加法运算
 select '123' + 90; 其中一方为字符型，试图将字符型数值转换成数值型，如果转换成功，则继续做加法运算
 select 'john' + 90; 如果转换失败，则将字符型数值转为0
 select null + 10; 只要其中一方为null，结果肯定为null
 */
# 案例：查询员工名和姓连接成一个字段，并显示为姓名
-- select last_name + first_name as name from employees;  -- 输出全为0
-- select last_name || ' ' || first_name as name from employees; --输出为0，oracle形式，mysql不适用
select concat('a', 'b', 'c') as result;
select concat(last_name, ' ', first_name) as name from employees;
# commission_pct有null值，会导致结果显示为null
...
# DML语言的学习
DML全称是Data Manipulation Language，有些书上把DQL和DML全称为DML。
...
# TCL语言的学习
TCL全称是Transaction Control Language
